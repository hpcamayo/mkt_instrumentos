...
test("Particular account navigation exposes only implemented Particular destinations", () => {
  ...
  assert.deepEqual(items.map((item) => item.label), ["Resumen", "Mis publicaciones", "Publicar instrumento", "Notificaciones", "Favoritos", "Perfil", "Seguridad"]);
...
test("Store Owner navigation changes safely when an owner-bound store exists", () => {
  ...
  assert.deepEqual(withoutStore.map((item) => item.label), ["Resumen", "Solicitud de tienda", "Notificaciones", "Favoritos", "Perfil", "Seguridad"]);
  const withStore = getAccountNavigationItems("store_owner", true);
  assert.deepEqual(withStore.map((item) => item.label), ["Resumen", "Mi tienda", "Inventario", "Publicar producto", "Estadísticas", "Notificaciones", "Favoritos", "Perfil", "Seguridad"]);
