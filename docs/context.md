...
- Sprint 4 is **CLOSED / ACCEPTED**: the owner reported all eight remaining production cases PASS on 2026-09-17 (`REV-014`, `PHOTO-015`, `AUTH-023`, `WA-001`–`WA-004`, `SANA-010`). Production source remains `427ac8e8aa514ae10a47c0a4d2eee3dfe827ccaa`. Sprint 5 is local implementation only; no production release is authorized.
...
- `/mi-cuenta/favoritos` and `/mi-cuenta/favoritos/[id]`: Sprint 5 local private saved history and current safe public/sold destination resolver; not deployed yet.
- `/api/account-navigation` and `/api/favorites`: Sprint 5 local trusted hydrated header state and bounded own-favorite state/mutations.
...
Analytics semantics: Particular summary aggregates all owned inventory, not its recent-five list. Lifetime views preserve the historical cache; 7/30-day views use recorded events without fabricated history. CTR = recorded views / impressions; contact rate = contacts / recorded views in one period. Zero denominator is `Sin datos`; unavailable aggregates are not zero. Active inventory requires active-parent visibility; sold counts are not verified sales. Store statistics support 0/7/30 days. Sprint 5 locally adds current favorites independent of period, period add/remove actions and additions/recorded-view rate. Revenue/transaction metrics and the full admin analytics hub remain absent.

Acceptance process: `acceptance/cases.tsv` is canonical and `acceptance/sprints.tsv` is the selective retrieval guide. Read only exact current-sprint IDs/domain prefixes, preserve unrelated rows/IDs, and validate TSV changes. Do not read or regenerate XLSX until the final V1 freeze gate. Sprint 4 owner production acceptance is closed; Sprint 5 local automated proof is not production owner acceptance.

Sprint 5 local architecture: shared root header/category shell surrounds all routes, while protected account navigation remains nested. Native brand search reuses `/listados` parsing and signed receipt analytics. Hydrated trusted account/favorite state is batched without public SSR Auth calls or a browser Auth SDK in the provider. Private own favorite relations, redacted unavailable history and listing-row-serialized atomic in-app price-drop notifications extend existing RLS and aggregate paths. Same-form duplicate-RUC retries release known rejection locking but never erase an earlier uncertain commit. New migration `20260917120000_sprint_5_favorites.sql` is local only. Preserve intentional local HEAD `6b608dd680d79c183a3bf3b134b289a43358560c`; do not push/deploy automatically. Sprint 6 is not started.
