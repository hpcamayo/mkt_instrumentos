...
- Sprints 1–5 are **CLOSED / ACCEPTED**. Sprint 6 production source is `9ea8e88a342edc4fb54d873e85974d576e42863f`; migration `20260918120000_sprint_6_transactions_reviews.sql` and the matching application were released on 2026-09-19, with automated production acceptance complete and owner wording acceptance still blocked only for `TX-013` and `REVW-020`. See `docs/sprint-6-verification.md` and `docs/sprint-6-production-verification.md`.
...
Acceptance process: `acceptance/cases.tsv` is canonical and `acceptance/sprints.tsv` is the selective retrieval guide. Read only exact current-sprint IDs/domain prefixes, preserve unrelated rows/IDs, and validate TSV changes. Do not read or regenerate XLSX until the final V1 freeze gate. Sprint 6 automated production evidence is recorded without claiming owner/manual acceptance; `TX-013` and `REVW-020` remain blocked for the owner.
