...
- `20260917120000_sprint_5_favorites.sql`: local-only private favorites, trusted add/remove actions, atomic price-drop notifications, safe history/destination RPCs and favorite aggregate extensions. All 16 migrations are applied locally; the Sprint 5 migration has not been applied to production.
...
Frozen V1 still requires exact-state search alerts/delivery deduplication, price-drop email delivery, verified transactions, two-way reviews, reports and centralized marketplace email delivery. Sprint 5 locally implements favorites/in-app price drops and real favorite analytics; production remains the accepted Sprint 4 schema. Transaction/review analytics are deferred, not satisfied by event scaffolding.

## Sprint 5 private favorites and price drops — local only
...
- `capture_favorite_event()`: INSERT/DELETE lifecycle actions emit `favorite_added`/`favorite_removed`, taking the actor from the trusted relation/auth context. Idempotent operations and cascading cleanup do not invent events. The event taxonomy is now 18 locally; the external service recorder still rejects forged favorite lifecycle events.
